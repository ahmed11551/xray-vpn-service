...
# Скрипт тестирования всех функций
...
test_payment_systems() {
    ...
    # Проверка YooKassa
    YOOKASSA_SHOP_ID=$(grep "YOOKASSA_SHOP_ID" /opt/xray-service/.env | cut -d= -f2)
    if [[ -n "$YOOKASSA_SHOP_ID" && "$YOOKASSA_SHOP_ID" != "your_yookassa_shop_id" ]]; then
        log "✅ YooKassa настроен"
    else
        warn "⚠️  YooKassa не настроен"
    fi
    
    # Проверка Robokassa
    ROBOKASSA_MERCHANT_LOGIN=$(grep "ROBOKASSA_MERCHANT_LOGIN" /opt/xray-service/.env | cut -d= -f2)
    if [[ -n "$ROBOKASSA_MERCHANT_LOGIN" && "$ROBOKASSA_MERCHANT_LOGIN" != "your_robokassa_login" ]]; then
        log "✅ Robokassa настроен"
    else
        warn "⚠️  Robokassa не настроен"
    fi
    ...
    if [[ $crypto_count -gt 0 ]]; then
        ...
        warn "⚠️  Криптовалютные кошельки не настроены"
    fi
}
...
# Основная функция
main() {
    log "🧪 Тестирование всех функций сервиса..."
