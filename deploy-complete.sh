...
# Полный скрипт развертывания Xray VLESS + Reality сервиса на OneDash
# Запустите этот скрипт на сервере: bash deploy-complete.sh
...
show_header() {
    ...
    echo "║                                                              ║"
    echo "║              Развертывание на OneDash                       ║"
    echo "║                                                              ║"
    echo "║         Коммерческий VPN сервис готов к запуску!           ║"
    echo "║                                                              ║"
    echo "╚══════════════════════════════════════════════════════════════╝"
    echo -e "${NC}"
}
...
update_system() {
    ...
    apt install -y git curl wget jq python3 python3-pip postgresql-client redis-tools unzip htop nano
...
EOF
    ...
    success "Базовые сервисы созданы"
}
...
EOF
    ...
    success "Скрипт генерации ключей создан"
}
...
generate_deployment_report() {
    local report_file="/opt/xray-service/deployment-report.txt"
    
    {
        echo "=== Xray VLESS + Reality Service Deployment Report ==="
        ...
        echo "• Prometheus: http://89.188.113.58:9090"
        echo ""
        
        echo "=== API Endpoints ==="
        echo "• GET /api/v1/servers - список серверов"
        echo "• POST /api/v1/configs/generate - генерация конфигурации"
        ...
        echo "3. Обновите BOT_TOKEN в .env файле"
    ...
    log "Отчет о развертывании сохранен в: $report_file"
    echo "$report_file"
}
...
main() {
    ...
    success "🎉 Сервис успешно развернут на OneDash!"
    ...
    success "Готово! Ваш коммерческий VPN сервис запущен! 🚀"
}
